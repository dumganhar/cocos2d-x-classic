...
CCPVRParzer::CCPVRParzer()
    ...
    , m_eCCFormat(kCCTexture2DPixelFormat_Default)
{
}
...
bool CCPVRParzer::unpackPVRv2Data(unsigned char* data, unsigned int len)
{
    ...
    const ccPVRTexturePixelFormatInfo *pixelFormatInfo = NULL;
    ...
    for (unsigned int i = 0; i < pvr2TableElements; i++)
    {
        ...
        {
            pixelFormatInfo = v2_pixel_formathash[i].pixelFormatInfo;
            m_eInternalFormat = pixelFormatInfo->internalFormat;
            m_ePixelDataformat = pixelFormatInfo->format;
            m_ePixelDataType = pixelFormatInfo->type;
            m_uBitsPerPixel = pixelFormatInfo->bpp;
            m_bCompressed = pixelFormatInfo->compressed;
            m_bHasAlpha = pixelFormatInfo->alpha;
            m_eCCFormat = pixelFormatInfo->ccPixelFormat;
            break;
        }
    }
    ...
    while (dataOffset < dataLength)
    {
        ...
        dataSize = widthBlocks * heightBlocks * ((blockSize  * pixelFormatInfo->bpp) / 8);
        ...
        CCAssert(m_uNumberOfMipmaps < CC_PVRMIPMAP_MAX, 
            "PVRParzer: Maximum number of mipmaps reached. Increase the CC_PVRMIPMAP_MAX value");
...
bool CCPVRParzer::unpackPVRv3Data(unsigned char* dataPointer, unsigned int dataLength)
{
    ...
    for(unsigned int i = 0; i < pvr3TableElements; i++)
    {
        ...
        {
            pixelFormatInfo = v3_pixel_formathash[i].pixelFormatInfo;
            m_eInternalFormat = pixelFormatInfo->internalFormat;
            m_ePixelDataformat = pixelFormatInfo->format;
            m_ePixelDataType = pixelFormatInfo->type;
            m_uBitsPerPixel = pixelFormatInfo->bpp;
    ...
    CCAssert(m_uNumberOfMipmaps < CC_PVRMIPMAP_MAX, "PVRParzer: Maximum number of mimpaps reached. Increate the CC_PVRMIPMAP_MAX value");
    ...
    for (unsigned int i = 0; i < m_uNumberOfMipmaps; i++)
    {
        ...
        case kPVR3TexturePixelFormat_BGRA_8888:
            ...
            {
                CCLOG("cocos2d: PVRParzer. BGRA8888 not supported on this device");
                return false;
            }
        default:
            blockSize = 1;
        ...
        dataOffset += packetLength;
        CCAssert(dataOffset <= dataLength, "PVRParzer: Invalid lenght");
...
bool CCPVRParzer::parzeData(const void *data, unsigned int dataLen)
{
    unsigned char *pvrData = (unsigned char *)data;
    m_uNumberOfMipmaps = 0;
    ...
    if (! (unpackPVRv2Data(pvrData, dataLen)  || unpackPVRv3Data(pvrData, dataLen)) )
    {
        return false;
    }
